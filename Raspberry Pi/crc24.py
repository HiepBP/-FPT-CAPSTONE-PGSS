...
def calculateDebug(data):
    result = CRC_24_INITIALIZATION
    print("Initial register: {}".format(result))
    for byte in data:
        print("Message byte: {}".format(byte))
        index = ((result >> 16) ^ byte) & 0xff
        print("Index {}".format(index))
        print("Table value: {}".format(CRC_24_TABLE[index]))
        result = (CRC_24_TABLE[index] ^ (result << 8)) & 0x00ffffff
        print("New register: {}".format(result))
    return result
